...
#
#     bars = [2, 5, 8, 11, 14]
#     plt.bar([i - 0.8 for i in bars], avg_capacity,  label=name + ' optimal')
#     # plt.bar(bars, avg_capacity_nopenalty,  label=name + ' optimal no penalty')
#     plt.bar([i  for i in bars], avg_beamwidth_capacity,  label=name + ' beamwidth')
...
for i, beamwidth_b in zip(range(3), beamwidths):
    ...
    avg_capacity = [sum(capacity[beamwidth_b][i]) / max(1, len(capacity[beamwidth_b][i])) for i in range(len(user))]
    avg_capacity2 = [sum(capacity2[beamwidth_b][i]) / max(1, len(capacity2[beamwidth_b][i])) for i in range(len(user))]
    avg_capacity3 = [sum(capacity3[beamwidth_b][i]) / max(1, len(capacity3[beamwidth_b][i])) for i in range(len(user))]

    bars = [2, 5, 8]
    plt.bar([i - 0.8 for i in bars], avg_capacity,  label=name + ' s = 1')
    # plt.bar(bars, avg_capacity_nopenalty,  label=name + ' optimal no penalty')
    plt.bar([i  for i in bars], avg_capacity2,  label=name + ' s = 2')
    plt.bar([i + 0.8 for i in bars], avg_capacity3,  label=name + ' s = 3')
...
#              label='$SNR - k = 5$')
#     plt.plot(user, [average(MC_SNR_heuristic_capacity[beamwidth][24][i]) for i in range(len(user))], '-1',
#              label='$All SNR$')
#     plt.legend()
#     plt.xlabel('Number of users')
#     plt.ylabel('Total channel capacity (Gbit/s/Hz)')
...
for i, beamwidth_b in zip(range(3), beamwidths):
    ...
    for j, bar in zip(range(len(user)), bars):
        ax.annotate("", xy=(bar - 0.7, avg_capacitylos[j]), xytext=(bar - 0.7, avg_capacity[j]),  arrowprops=dict(arrowstyle="->"))
        ax.annotate(f"${round((avg_capacity[j] - avg_capacitylos[j])/avg_capacity[j] * 100,2)}\\%$", xy=(bar - 0.6, avg_capacity[j] - (avg_capacity[j] - avg_capacitylos[j])/2), fontsize = size)
    ...
    for j, bar in zip(range(len(user)), bars):
        ax.annotate("", xy=(bar + 0.6, avg_beamwidth_capacitylos[j]), xytext=(bar + 0.6, avg_beamwidth_capacity[j]),  arrowprops=dict(arrowstyle="->"))
        ax.annotate(f"${round((avg_beamwidth_capacity[j] - avg_beamwidth_capacitylos[j])/avg_beamwidth_capacity[j] * 100,2)}\\%$", xy=(bar + 0.7, avg_beamwidth_capacity[j] - (avg_beamwidth_capacity[j] - avg_beamwidth_capacitylos[j])/2), fontsize = size)
