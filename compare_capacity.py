...
failed_link_constraints_beamwidth_heuristic = []
failed_snr_constraints_beamwidth_heuristic = []
...
for number_of_users in [100, 300, 500]:
    ...
    failed_link_constraints_beamwidth_heuristic.append(pickle.load(open(str('Data/failed_link_constraints_iteration_'+ str(max_iterations) + 'beamwidth_heuristic_users='  + str(
            number_of_users) + 'beamwidth_u=' + str(np.degrees(beamwidth_u)) + 'beamwidth_b=' + str(
            np.degrees(beamwidth_b)) + '.p'), 'rb')))
    failed_snr_constraints_beamwidth_heuristic.append(pickle.load(open(str('Data/failed_sinr_constraints_iteration_'+ str(max_iterations) + 'beamwidth_heuristic_users='  + str(
            number_of_users) + 'beamwidth_u=' + str(np.degrees(beamwidth_u)) + 'beamwidth_b=' + str(
            np.degrees(beamwidth_b)) + '.p'), 'rb')))
    ...
    if Interference:
        name = str('until_iteration_' + str(max_iterations) + 'users=' + str(number_of_users) + 'beamwidth_u=' + str(
            np.degrees(beamwidth_u)) + 'beamwidth_b=' + str(np.degrees(beamwidth_b)))
    else:
...
for i in range(len(user)):
    ...
    print('Average number of disconnected users (beamwidth heuristic):', sum(failed_link_constraints_beamwidth_heuristic[i])/(len(failed_link_constraints_beamwidth_heuristic) * user[i]), 'out of', user[i], 'users', 'in', sum([1 for j in range(len(failed_link_constraints_beamwidth_heuristic[i])) if failed_link_constraints_beamwidth_heuristic[i][j] > 0])/len(failed_link_constraints_beamwidth_heuristic[i]) * 100, 'percent of the iterations')
...
print(failed_snr_constraints_beamwidth_heuristic[0])

fig, ax = plt.subplots()
plt.plot(user, [np.count_nonzero(failed_snr_constraints_beamwidth_heuristic[i])/len(failed_snr_constraints_beamwidth_heuristic[i])*100 for i in range(len(user))], label = 'Beamwidth - SNR too low', marker = 'o')
plt.plot(user, [np.count_nonzero(failed_link_constraints_beamwidth_heuristic[i])/len(failed_link_constraints_beamwidth_heuristic[i])*100 for i in range(len(user))], label = 'Beamwidth - disconnected', marker = 'o')
...
plt.xlabel('Number of users')
plt.ylabel('% of iterations with failed SNR-min constraint/disconnected users')
plt.legend()
plt.show()

fig, ax = plt.subplots()
plt.plot(user, [np.sum(failed_snr_constraints_beamwidth_heuristic[i])/(len(failed_snr_constraints_beamwidth_heuristic[i]) ) for i in range(len(user))], label = 'Beamwidth', marker = 'o')
# plt.plot(user, [np.sum(failed_link_constraints_beamwidth_heuristic[i])/(len(failed_link_constraints_beamwidth_heuristic[i]) ) for i in range(len(user))], label = 'Beamwidth - disconnected', marker = 'o')

plt.plot(user, [np.sum(failed_snr_constraints_SC_closest_heuristic[i])/(len(failed_snr_constraints_SC_closest_heuristic[i]) ) for i in range(len(user))], label = 'SC - closest', marker = 'o')
plt.plot(user, [np.sum(failed_snr_constraints_MC2_closest_heuristic[i])/(len(failed_snr_constraints_MC2_closest_heuristic[i]) ) for i in range(len(user))], label = 'MC2 - closest', marker = 'o')
plt.plot(user, [np.sum(failed_snr_constraints_MC3_closest_heuristic[i])/(len(failed_snr_constraints_MC3_closest_heuristic[i]) ) for i in range(len(user))], label = 'MC3 - closest', marker = 'o')
plt.plot(user, [np.sum(failed_snr_constraints_MC4_closest_heuristic[i])/(len(failed_snr_constraints_MC4_closest_heuristic[i]) ) for i in range(len(user))], label = 'MC4 - closest', marker = 'o')
plt.plot(user, [np.sum(failed_snr_constraints_MC5_closest_heuristic[i])/(len(failed_snr_constraints_MC5_closest_heuristic[i]) ) for i in range(len(user))], label = 'MC5 - closest', marker = 'o')
plt.xlabel('Number of users')
plt.ylabel('average number of failed SNR-min links')
plt.legend()
plt.show()